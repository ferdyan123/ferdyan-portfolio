...
export default function About() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(valuesRef.current.filter(Boolean), { opacity: 0, y: 30 }, {
    ...
    <section id="about" ref={sectionRef} className="relative py-28 px-6 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-3 mb-16">
          ...
          <span className="font-mono text-muted text-sm tracking-widest uppercase">{a.eyebrow}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div ref={leftRef} style={{ opacity: 0 }}>
            <h2 className="text-4xl md:text-5xl font-bold leading-tight mb-6 tracking-tight">
              {a.headlineLine1}{' '}
              <span className="text-accent">{a.headlineAccent}</span>
            </h2>

            <p className="text-muted leading-relaxed mb-6 text-base md:text-lg">
              {a.paragraph1Pre}
              <span className="text-white font-semibold">{a.paragraph1Name}</span>
              {a.paragraph1Mid}
              <span className="text-white font-medium">{a.paragraph1Bold}</span>
              {a.paragraph1End}
            </p>
          ...
          <div ref={rightRef} style={{ opacity: 0 }} className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl border border-accent/20 bg-accent/5 mb-2">
              <div className="w-8 h-8 rounded-lg bg-accent/20 flex items-center justify-center text-lg flex-shrink-0">🤖</div>
              <div>
                <p className="text-white font-semibold text-sm">{a.aiBadgeTitle}</p>
                <p className="text-muted text-xs mt-0.5">{a.aiBadgeDesc}</p>
              </div>
            </div>

            {/* Value cards dari content.js */}
            {a.values.map((v, i) => (
              <div
                key={i}
            ...
            <div className="mt-4 p-4 rounded-xl border border-border/50">
              <p className="font-mono text-xs text-muted/60 leading-relaxed">
                <span className="text-accent/60">{a.philosophyLabel}</span>
                <br />
